...
int get_number(string enter)
{
    ...
    if ((middle[0] == 'w') || (wild_check_a == "wild") || (wild_check_b == "wildcard"))
        output = 97;
    ...
    if ((middle[0] == 's') || (skip_check_a == "skip") || (skip_check_b == "skipcard"))
        output = 105;
...
int phase_1_p1()
{
    ...
    if (check_set(pool, 3) > 0)
    {
        cout<<"Correct! Enter the next set of 3.\n";
        phase1[1].set_num = check_set(pool, 3);
    }
    else
    {
    ...
    if (check_set(pool, 3) > 0)
    {
        ...
        phase1[2].set_num = check_set(pool, 3);
    }
    else
    {
        cout<<"Wrong phase! Please reenter the whole phase!\n";
